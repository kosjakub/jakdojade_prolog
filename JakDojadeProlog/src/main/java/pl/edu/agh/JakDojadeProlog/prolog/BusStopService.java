...
public class BusStopService {
    ...
    public List<Result> getAllRoutes(String startBusStop, String endBusStop) {
        ...
        Variable ListOfLinesReturn = new Variable("ListOfLinesReturn");
        Query q =
                new Query(
                        "route",
                        new Term[]{new Atom(startBusStop), new Atom(endBusStop), Route, Time, ListOfLinesReturn}
                );
        ...
        for (Map<String, Term> solution : solutions) {
            Result result = new Result(solution.get("Time").bigValue().toString(),
                    Arrays.stream(solution.get("Route").toTermArray())
                            ...
                            .map(Term::name)
                            .collect(Collectors.toList()),
                    Arrays.stream(solution.get("ListOfLinesReturn").toTermArray())
                            .map(Term::toTermArray)
                            .map(e -> Arrays.stream(e).map(Term::name).collect(Collectors.toList()))
